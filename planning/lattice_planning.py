...
#print( '\n\nnext state: ' ,next_state(mission["start"]["id"], world, mp) )
...
#First is translation and second is the angle
cost_to_go = lambda x, xg: np.linalg.norm(world.st_sp[0:2, x] - world.st_sp[0:2, xg]) + abs(world.st_sp[2, x] - world.st_sp[2, xg])
...
titles = ['breadth_first', 'depth_first', 'dijkstra', 'astar', 'best_first']
for i in range(5):

    p, sp = mp.plan_to_path( start, res[i] )
    ...
    print(res[i]['num_expanded_nodes'])
    #print('path time: ', res[i]['time'])
