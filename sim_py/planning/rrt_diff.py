...
def rrt_diff(start, goal, u_c, sim, world, opts):
    ...
    def distance_fcn(x1, x2):
        """Function for computing the distance between states x1 and x2, where 
            ...
            all states equally"""
        d2 = x1 - x2
        return np.sqrt(d2[0]**2 + d2[1]**2 + 2*d2[2]**2)
    ...
    print( 'state_trajectories: ', state_trajectories )
    # YOUR CODE HERE

    for i in range(opts['K']):
        qrand = sample_free()
        qnearestidx = nearest(qrand) # id of nearest sampled node
        #print('nodes: ', nodes)
        qnearest = [nodes[0, qnearestidx], nodes[1, qnearestidx], nodes[2, qnearestidx]] #Coordinates of above node id
        valid_new_paths, dist_to_x_rand = steer_candidates(qnearest, qrand) #Finds valid paths and distance between sampled node and goal
        

        #state_trajectories

        #Rethink this!  
        if valid_new_paths and dist_to_x_rand !=-1:

            #print('valid_new_paths: ', valid_new_paths)
            #print('dist_to_x_rand: ', dist_to_x_rand)
            min_idx = np.argmin(dist_to_x_rand)
            min_dist = dist_to_x_rand[min_idx]

            trajectory = valid_new_paths[min_idx] #Shortest path

            #print('\ntrajectory: ', trajectory)
            tmp = [ [trajectory[0][-1]], [trajectory[1][-1]], [trajectory[2][-1]] ]
            #print('\ntmp: ', tmp)
            nodes = np.append(nodes, tmp, axis=1)
            parents = np.append(parents, qnearestidx)
            
            tmp2 = np.transpose([ trajectory[0], trajectory[1], trajectory[2] ])
            #print(len(tmp2))
            tmp2 = tmp2.reshape(len(tmp2),3)
            #print('\nstate_trajectories: ', state_trajectories[0])
            #print('\ntmp: ', len(tmp2))
            state_trajectories.append([ tmp2 ])
            #print('size: ', state_trajectories)
            #state_trajectories = np.vstack( [state_trajectories, tmp2 ] )
            
            
        if np.linalg.norm(tmp - goal) < opts['eps']:
            print('avbryt')
            break
...
opts = {'beta': 0.05,  # Probability of selecting goal state as target state
        ...
        'eps': 2,  # Threshold for stopping the search (negative for full search)
...
searched = []
idx = goal_idx
for j in parents[1:-1]:
    idx = j
    while idx != 0 or idx not in searched:
        traj_i = state_trajectories[idx]
        drawlines.append(np.transpose(traj_i)[0])
        drawlines.append(np.transpose(traj_i)[1])
        searched.append(idx)
        idx = parents[idx]
plt.plot(*drawlines, color='g', lw=1)


print('number nodes: ', len(nodes[0]))
drawlines = []
idx = goal_idx
while idx != 0:
    traj_i = state_trajectories[idx][0]
    #print('\ntraj_i: ', traj_i)
    #print('traj_i[0]: ', np.transpose(traj_i)[0] )
    drawlines.append(np.transpose(traj_i)[0])
    drawlines.append(np.transpose(traj_i)[1])
    idx = parents[idx]
