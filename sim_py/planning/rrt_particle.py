...
#from HI2_PlanningWithDifferentialConstraints.python.world import obstacle_check
from os import path
import numpy as np
...
def get_path():
    ...
    def rrt_particle(start, goal, world, opts):
        ...
        for i in range(opts['K']):
            qrand = sample_free()
            qnearestidx = nearest(qrand)

            qnearest = [nodes[0, qnearestidx], nodes[1, qnearestidx]]
            qnew = steer(qnearest, qrand)
            
            path = np.array([np.linspace(qnew[0],qnearest[0],6), np.linspace(qnew[1],qnearest[1],6)])

            if world.obstacle_free(path):

                nodes = np.append(nodes, qnew.reshape(2,1), axis=1)
                parents = np.append(parents, qnearestidx)

            if np.linalg.norm(qnew - goal) < opts['eps']:
                break
    ...
    opts = {'beta': 0.03,  # Probability of selecting goal state as target state in the sample
            'lambda': 0.8,  # Step size
            'eps': 0.2,  # Threshold for stopping the search (negative for full search)
            'K': 4000}     # Maximum number of iterations, if eps < 0
    ...
    while idx != 0:
        ...
        drawlines.append(ll[0])
        drawlines.append(ll[1])
        idx = parents[idx] #step back
        nodes_in_path = nodes_in_path + 1

        path_coord.append([ ll[0][1]/4, ll[1][1]/4, 0.7 ])

    plt.plot(*drawlines, color='b', lw=2)
    print('nodes_in_path: ', nodes_in_path)
